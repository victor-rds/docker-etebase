...
ADD https://github.com/etesync/server-skeleton/archive/master.tar.gz $BASE_DIR
...
RUN set -e && \
    ...
    apk add --no-cache openssl libressl && \
...
# Fix permissions
RUN chown -h etesync:etesync $BASE_DIR/static && \
